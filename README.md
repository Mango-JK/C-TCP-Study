...
> 네트워크 프로그래밍이란, 
>
> ​	네트워크로 연결되어 있는 서로 다른 두 컴퓨터가 데이터를 주고받을 수 있도록 하는 것이다.
...
처음 접하는 것은 TCP 소켓이라는 것이다. 전화기에 비유할 수 있다.

> 1.  **전화기의 장만**에 비유되는 `socket 함수 호출`의 이해를 위한 대화
>    - **Question**	"전화를 받으려면 무엇이 필요한가?"
>    - **Answer**         전화기!
...
> 2. **전화번호의 부여**에 비유되는 `bind 함수호출`의 이해를 위한 대화
...
> 3. **전화기의 케이블 연결**에 비유되는 `listen 함수호출`의 이해를 위한 대화
>
> - **Question**	"전화기를 연결만 하면 되나요?"
> - **Answer**       "네 연결만 하면 걸려오는 전화를 받을 수 있습니다."
...
> 4. **수화기를 드는 것**에 비유되는 `accept 함수호출`의 이해를 위한 대화
>
> - **Question** 	"전화벨이 울립니다. 어떻게 해야 되나요?"
> - **Answer**       "전화를 받으세요!"
...
```c
// 지금까지 설명한 내용을 정리해보면, 네트워크 프로그래밍에서 연결요청을 허용하는 소켓의 생성과정을 다음과 같이 정리할 수 있다.
